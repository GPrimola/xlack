defmodule Xlack.Sends do
  ...
  @doc """
  Notifies Xlack that the current user is typing in `channel`.
  """
  ...
  defp open_im_channel(token, user_id, on_success, on_error) do
    im_open =
      with url <- Application.get_env(:xlack, :url, "https://slack.com") <> "/api/im.open",
           headers <- {:form, [token: token, user: user_id]},
           options <-
             Application.get_env(:xlack, :web_http_client_opts, [{"Content-Type", "utf-8"}]) do
