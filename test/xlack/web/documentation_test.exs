defmodule Xlack.Web.DocumentationTest do
  use ExUnit.Case
  alias Xlack.Web.Documentation
  ...
  describe "new/2" do
    test "takes a documentation and filename, returns a module & function description" do
      ...
      module_functions = Xlack.Web.Team.__info__(:functions)
    ...
    test "accepts versioned endpoints" do
      file_content =
        ...
        |> Path.join("slack")
        |> Path.join("docs")
        |> Path.join("oauth.v2.access.json")
        |> File.read!()
        |> Jason.decode!(%{})
      ...
      module_functions = Xlack.Web.Oauth.V2.__info__(:functions)
